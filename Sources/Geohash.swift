...
public enum Geohash {
     ...
     */
    public static func geohashQueries(center: CLLocation, radius: CLLocationDistance) -> [ClosedRange<String>] {
        let radius = radius * 1000
        let queryBits = max(1, self.boundingBoxBits(coordinate: center.coordinate, size: radius))
        let geohashPrecision = Int(ceil(Double(queryBits) / Double(Constants.bitsPerChar)))
        let coordinates = self.boundingBoxCoordinates(center: center.coordinate, radius: radius)
        let queries = coordinates.map {
            self.geohashQuery(geohash: self.encodeGeohash(location: $0, precision: geohashPrecision), bits: queryBits)
        }
        // remove duplicates
        do {
     ...
     */
    public static func encodeGeohash(location: CLLocation, precision: Int = Constants.geohashPrecision) -> String {
        let precision: Int = {
            if precision <= 0 {
                return 1
            }
    ...
    // MARK: - Internal
    static func boundingBoxBits(coordinate: CLLocationCoordinate2D, size: CLLocationDistance) -> Int {
        ...
        let bitsLatitude = Int(floor(self.latitudeBits(for: size))) * 2
        let bitsLongitudeNorth = Int(floor(self.longitudeBits(for: size, latitude: latitudeNorth))) * 2 - 1
        let bitsLongitudeSouth = Int(floor(self.longitudeBits(for: size, latitude: latitudeSouth))) * 2 - 1
     ...
     */
    static func boundingBoxCoordinates(center: CLLocationCoordinate2D, radius: CLLocationDistance) -> [CLLocation] {
        ...
        let longitudeDegreesNorth = self.metersToLongitudeDegrees(distance: radius, latitude: latitudeNorth)
        let longitudeDegreesSouth = self.metersToLongitudeDegrees(distance: radius, latitude: latitudeSouth)
        ...
        return [
            ...
            CLLocation(latitude: center.latitude, longitude: self.wrapLongitude(center.longitude - longitudeDegrees)),
            CLLocation(latitude: center.latitude, longitude: self.wrapLongitude(center.longitude + longitudeDegrees)),
            CLLocation(latitude: latitudeNorth, longitude: center.longitude),
            CLLocation(latitude: latitudeNorth, longitude: self.wrapLongitude(center.longitude - longitudeDegrees)),
            CLLocation(latitude: latitudeNorth, longitude: self.wrapLongitude(center.longitude + longitudeDegrees)),
            CLLocation(latitude: latitudeSouth, longitude: center.longitude),
            CLLocation(latitude: latitudeSouth, longitude: self.wrapLongitude(center.longitude - longitudeDegrees)),
            CLLocation(latitude: latitudeSouth, longitude: self.wrapLongitude(center.longitude + longitudeDegrees))
        ]
    }
     ...
     */
    /***/
    static func geohashQuery(geohash: String, bits: Int) -> ClosedRange<String> {
     ...
     */
    static func latitudeBits(for resolution: Double) -> Double {
    ...
    /**
     ...
     - returns: The bits necessary to reach a given resolution, in meters.
     */
    static func longitudeBits(for resolution: Double, latitude: Double) -> Double {
        let degrees = self.metersToLongitudeDegrees(distance: resolution, latitude: latitude)
     ...
     */
    static func metersToLongitudeDegrees(distance: Double, latitude: Double) -> Double {
        let radians = self.degreesToRadians(degrees: latitude)
     ...
     */
    /***/
    static func wrapLongitude(_ longitude: CLLocationDegrees) -> CLLocationDegrees {
